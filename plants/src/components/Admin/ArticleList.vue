        ...
        <div class="q-pa-md col-xs-12 col-sm-6 col-md-4 col-lg-3 grid-style-transition"
          ...
          <div class="q-pa-md row items-start q-gutter-md">
            <q-card class="my-card" flat bordered>
              ...
              <q-card-section>
                <div class="text-overline text-orange-9">Id Articulo: {{ props.row.article_id }}</div>
                <div class="text-overline text-orange-9">{{ props.row.category }}</div>
                <div class="tex-bold q-mt-sm">Planta: {{ props.row.plant }}</div>
              </q-card-section>

              <q-card-actions>
                <router-link :to="{ name: '/edit-article/', params: { id: props.row.article_id } }" > <q-btn flat color="primary" label="Editar" />  </router-link>
                <q-btn flat color="red" label="Eliminar" @click="deleteArticle(props.row.article_id)" />
              </q-card-actions>
            </q-card>
          </div>
        </div>
...
const get = async () => {
  const query = await fetch(host + 'api/articles/')
...
const columns = [
  ...
  { name: 'fat', label: 'Planta', field: 'plant', align: 'center' },
...
const deleteArticle = async (id: string) => {
  const request = await fetch(host + `api/articles/${id}/`,
    {
      method: 'DELETE',
      headers: {
